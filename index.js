...
(async () => {
    try {
        ...
        // Rota para buscar a logo da empresa
        app.get('/api/empresa/logo', async (req, res) => {
            try {
                const empresa = await Empresa.findOne({ attributes: ['logo'] });
                if (empresa) {
                    res.json({ logo: empresa.logo });
                } else {
                    res.status(404).json({ error: 'Empresa não encontrada' });
                }
            } catch (error) {
                console.error('Erro ao buscar a logo da empresa:', error);
                res.status(500).json({ error: 'Erro ao buscar a logo da empresa' });
            }
        });
        ...
        // Rota para buscar a foto do usuário
        app.get('/api/usuarios/foto/:username', async (req, res) => {
            ...
            try {
                ...
                res.status(500).json({ error: 'Erro ao buscar a foto do usuário' });
            }
        });
